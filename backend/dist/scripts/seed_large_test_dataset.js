...
async function upsertUser(client, data) {
    const existing = await client.query('SELECT id FROM identity.users WHERE email = $1 OR (phone IS NOT NULL AND phone = $2) LIMIT 1', [data.email, data.phone]);
    if (existing.rows.length > 0) {
        const id = existing.rows[0].id;
        await client.query('UPDATE identity.users SET email = $1, phone = $2, password_hash = $3, first_name = $4, last_name = $5, status = $6, deleted_at = NULL WHERE id = $7', [data.email, data.phone, data.passwordHash, data.firstName, data.lastName, 'ACTIVE', id]);
        return id;
    }
    const insertRes = await client.query('INSERT INTO identity.users (email, phone, password_hash, first_name, last_name, status) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id', [data.email, data.phone, data.passwordHash, data.firstName, data.lastName, 'ACTIVE']);
...
async function seedLargeDataset() {
    ...
    for (let sIdx = 0; sIdx < SCHOOLS_DATA.length; sIdx++) {
        ...
        const schoolIndex = sIdx + 1;
        console.log(`\n========================================`);
        console.log(`Processing School [${schoolIndex}/5]: ${sData.name} (${sData.code})`);
        console.log(`========================================`);
        const schoolRes = await client.query(`INSERT INTO school.schools (name, code, email, phone, address_line1, city, status)
       ...
       RETURNING id;`, [sData.name, sData.code, sData.email, sData.phone, sData.address, sData.city]);
        const schoolId = schoolRes.rows[0].id;
        const branchRes = await client.query(`INSERT INTO school.school_branches (school_id, name, code, address_line1, city, status)
       VALUES ($1, $2, $3, $4, $5, 'ACTIVE')
       ON CONFLICT (school_id, code) DO UPDATE SET name = EXCLUDED.name
       RETURNING id;`, [schoolId, `${sData.city} Main Campus`, 'MAIN', sData.address, sData.city]);
        const branchId = branchRes.rows[0].id;
        const ayRes = await client.query(`INSERT INTO school.academic_years (school_id, name, start_date, end_date, is_current, status)
       ...
       RETURNING id;`, [schoolId]);
        ...
        for (const c of STANDARD_CLASSES) {
            const cRes = await client.query(`INSERT INTO school.classes (school_id, name, code, display_order, status)
         VALUES ($1, $2, $3, $4, 'ACTIVE')
         ON CONFLICT (school_id, code) DO UPDATE SET name = EXCLUDED.name, display_order = EXCLUDED.display_order
         RETURNING id;`, [schoolId, c.name, c.code, c.order]);
            const classId = cRes.rows[0].id;
            const secARes = await client.query(`INSERT INTO school.sections (school_id, academic_year_id, branch_id, class_id, name, code, capacity, display_order, status)
         ...
         RETURNING id;`, [schoolId, academicYearId, branchId, classId]);
            const secBRes = await client.query(`INSERT INTO school.sections (school_id, academic_year_id, branch_id, class_id, name, code, capacity, display_order, status)
        ...
        for (const sub of STANDARD_SUBJECTS) {
            const subRes = await client.query(`INSERT INTO school.subjects (school_id, name, code, subject_type, display_order, status)
         VALUES ($1, $2, $3, $4, $5, 'ACTIVE')
         ON CONFLICT (school_id, code) DO UPDATE SET name = EXCLUDED.name
         RETURNING id;`, [schoolId, sub.name, sub.code, sub.type, sub.order]);
        ...
        for (const [cCode, cInfo] of classMap.entries()) {
            for (const [subCode, subId] of subjectMap.entries()) {
                const csRes = await client.query(`INSERT INTO school.class_subjects (academic_year_id, class_id, subject_id, is_optional, display_order, status)
           VALUES ($1, $2, $3, false, 1, 'ACTIVE')
           ON CONFLICT (academic_year_id, class_id, subject_id) DO UPDATE SET status = 'ACTIVE'
           RETURNING id;`, [academicYearId, cInfo.classId, subId]);
        ...
        for (const st of staffAccounts) {
            ...
            if (roleId) {
                await client.query(`INSERT INTO identity.user_school_roles (user_id, school_id, role_id, status)
           VALUES ($1, $2, $3, 'ACTIVE')
           ON CONFLICT (user_id, school_id, role_id) DO NOTHING;`, [uId, schoolId, roleId]);
            }
        }
        ...
        for (let tIdx = 0; tIdx < TEACHER_PROFILES.length; tIdx++) {
            ...
            if (tRoleId) {
                await client.query(`INSERT INTO identity.user_school_roles (user_id, school_id, role_id, status)
           VALUES ($1, $2, $3, 'ACTIVE')
           ON CONFLICT (user_id, school_id, role_id) DO NOTHING;`, [tUserId, schoolId, tRoleId]);
            }
            ...
            if (secId && csId) {
                await client.query(`INSERT INTO school.section_subject_teacher_assignments (
             school_id, academic_year_id, section_id, class_subject_id, user_id, status
           ) VALUES ($1, $2, $3, $4, $5, 'ACTIVE')
           ON CONFLICT DO NOTHING;`, [schoolId, academicYearId, secId, csId, tUserId]);
                if (t.isClassTeacher) {
                    await client.query(`INSERT INTO school.section_teacher_assignments (
               school_id, academic_year_id, section_id, user_id, status
             ) VALUES ($1, $2, $3, $4, 'ACTIVE')
             ON CONFLICT DO NOTHING;`, [schoolId, academicYearId, secId, tUserId]);
                }
            }
            schoolTeachers.push({
                name: `${t.first} ${t.last}`,
        ...
        for (let cIdx = 0; cIdx < STANDARD_CLASSES.length; cIdx++) {
            ...
            for (let sIdx = 1; sIdx <= studentsInThisClass; sIdx++) {
                ...
                const stRes = await client.query(`INSERT INTO school.students (school_id, admission_number, first_name, last_name, gender, blood_group, status)
           VALUES ($1, $2, $3, $4, $5, 'B+', 'ACTIVE')
           ON CONFLICT (school_id, admission_number) DO UPDATE SET first_name = EXCLUDED.first_name
           RETURNING id;`, [schoolId, admNo, firstName, lastName, isMale ? 'MALE' : 'FEMALE']);
                const studentId = stRes.rows[0].id;
                totalStudentsInserted++;
                await client.query(`INSERT INTO school.student_enrollments (academic_year_id, student_id, section_id, roll_number, status)
           VALUES ($1, $2, $3, $4, 'ACTIVE')
           ON CONFLICT (academic_year_id, student_id) DO UPDATE SET section_id = EXCLUDED.section_id, roll_number = EXCLUDED.roll_number;`, [academicYearId, studentId, targetSecId, rollNo]);
                ...
                if (guardianRoleId && parentUserId) {
                    await client.query(`INSERT INTO identity.user_school_roles (user_id, school_id, role_id, status)
             VALUES ($1, $2, $3, 'ACTIVE')
             ON CONFLICT (user_id, school_id, role_id) DO NOTHING;`, [parentUserId, schoolId, guardianRoleId]);
                }
                const occupation = OCCUPATIONS[studentCounter % OCCUPATIONS.length];
                const gRes = await client.query(`INSERT INTO school.guardians (school_id, user_id, first_name, last_name, phone, email, occupation, status)
           VALUES ($1, $2, $3, $4, $5, $6, $7, 'ACTIVE')
           RETURNING id;`, [schoolId, parentUserId, parentFirst, lastName, parentPhone, parentEmail, occupation]);
                const guardianId = gRes.rows[0].id;
                await client.query(`INSERT INTO school.student_guardians (school_id, student_id, guardian_id, relationship_type, is_primary_contact, is_emergency_contact, can_pickup, status)
           VALUES ($1, $2, $3, 'FATHER', true, true, true, 'ACTIVE')
           ON CONFLICT DO NOTHING;`, [schoolId, studentId, guardianId]);
                const attStatuses = ['PRESENT', 'PRESENT', 'PRESENT', studentCounter % 9 === 0 ? 'ABSENT' : studentCounter % 7 === 0 ? 'LATE' : 'PRESENT', 'PRESENT'];
                for (let d = 0; d < 5; d++) {
                    await client.query(`INSERT INTO school.attendance (school_id, academic_year_id, student_id, section_id, date, status, marked_by)
             VALUES ($1, $2, $3, $4, CURRENT_DATE - ($5 || ' days')::interval, $6, (SELECT id FROM identity.users WHERE email = $7 LIMIT 1))
             ON CONFLICT (student_id, academic_year_id, date) WHERE class_subject_id IS NULL AND deleted_at IS NULL DO UPDATE SET status = EXCLUDED.status;`, [schoolId, academicYearId, studentId, targetSecId, d, attStatuses[d], sData.adminEmail]);
                }
                schoolStudents.push({
                    id: studentId,
        ...
        for (const t of schoolTeachers.slice(0, 10)) {
            ...
            if (secId && csId) {
                await client.query(`INSERT INTO school.homework (school_id, academic_year_id, section_id, class_subject_id, teacher_id, title, description, assigned_date, due_date, status)
        ...
        const examRes = await client.query(`INSERT INTO school.exams (school_id, academic_year_id, name, code, start_date, end_date, status)
       VALUES ($1, $2, 'Mid-Term Periodic Assessment 2026', 'MIDTERM_2026', CURRENT_DATE - 20, CURRENT_DATE - 10, 'COMPLETED')
       ON CONFLICT DO NOTHING
       RETURNING id;`, [schoolId, academicYearId]);
        ...
        if (examId) {
            ...
            for (const csId of [csMath8, csSci8]) {
                if (!csId)
                    continue;
                const esRes = await client.query(`INSERT INTO school.exam_subjects (exam_id, class_subject_id, exam_date, max_marks, passing_marks, status)
           VALUES ($1, $2, CURRENT_DATE - 15, 100, 33, 'ACTIVE')
           ON CONFLICT (exam_id, class_subject_id) DO NOTHING
           RETURNING id;`, [examId, csId]);
                ...
                if (examSubjectId) {
                    ...
                    for (let stIdx = 0; stIdx < class8Students.length; stIdx++) {
                        ...
                        await client.query(`INSERT INTO school.student_marks (school_id, exam_subject_id, student_id, marks_obtained, is_absent, grade, remarks, entered_by)
               VALUES ($1, $2, $3, $4, false, $5, 'Good consistent performance', (SELECT id FROM identity.users WHERE email = $6 LIMIT 1))
               ON CONFLICT (exam_subject_id, student_id) DO UPDATE SET marks_obtained = EXCLUDED.marks_obtained;`, [schoolId, examSubjectId, st.id, marks, grade, sData.adminEmail]);
                    }
                }
            }
        }
        ...
        for (const n of noticeDefs) {
            await client.query(`INSERT INTO school.notices (school_id, academic_year_id, title, content, published_by, target_audience, status)
         VALUES ($1, $2, $3, $4, (SELECT id FROM identity.users WHERE email = $5 LIMIT 1), $6, 'PUBLISHED')
         ON CONFLICT DO NOTHING;`, [schoolId, academicYearId, n.title, n.content, sData.adminEmail, n.audience]);
        }
        await client.query(`INSERT INTO school.events (school_id, academic_year_id, title, description, start_time, end_time, location, is_holiday, target_audience, created_by)
       VALUES 
       ...
       ON CONFLICT DO NOTHING;`, [schoolId, academicYearId, sData.adminEmail]);
...
function generatePdfReport(schools, outputPath) {
...
//# sourceMappingURL=seed_large_test_dataset.js.map