    ...
    for (const sData of SCHOOLS_DATA) {
       ...
       RETURNING id;`, [schoolId]);
        ...
        for (const st of staffAccounts) {
            const uRes = await client.query(`INSERT INTO identity.users (email, phone, password_hash, first_name, last_name, status)
         VALUES ($1, $2, $3, $4, '', 'ACTIVE')
         ON CONFLICT DO NOTHING
         RETURNING id;`, [st.email, st.phone, sharedPasswordHash, st.name]);
            totalUsersInserted++;
        ...
        for (let tIdx = 0; tIdx < teacherDefs.length; tIdx++) {
            ...
            const tUserRes = await client.query(`INSERT INTO identity.users (email, phone, password_hash, first_name, last_name, status)
         VALUES ($1, $2, $3, $4, $5, 'ACTIVE')
         ON CONFLICT DO NOTHING
         RETURNING id;`, [tEmail, tPhone, sharedPasswordHash, t.first, t.last]);
            totalUsersInserted++;
        ...
        const classesToPopulate = STANDARD_CLASSES.filter((c) => c.order >= 4);
        ...
        for (let cIdx = 0; cIdx < classesToPopulate.length; cIdx++) {
           ...
           ON CONFLICT (academic_year_id, student_id) DO UPDATE SET section_id = EXCLUDED.section_id, roll_number = EXCLUDED.roll_number;`, [academicYearId, studentId, targetSecId, rollNo]);
                const parentUserRes = await client.query(`INSERT INTO identity.users (email, phone, password_hash, first_name, last_name, status)
           VALUES ($1, $2, $3, $4, $5, 'ACTIVE')
           ON CONFLICT DO NOTHING
           RETURNING id;`, [parentEmail, parentPhone, sharedPasswordHash, parentFirst, lastName]);
                totalUsersInserted++;
           ...
           ON CONFLICT DO NOTHING;`, [schoolId, studentId, guardianId]);
             ...
             ON CONFLICT DO NOTHING;`, [schoolId, academicYearId, studentId, targetSecId, d, attStatuses[d], sData.adminEmail]);
                }
                schoolStudents.push({
...
function generatePdfReport(schools, outputPath) {
    return new Promise((resolve, reject) => {
        ...
        doc.fontSize(9).font('Helvetica').fillColor(TEXT_MUTED).text(`• Total Schools: 5 Institutional Tenants\n• Total Active Students: 525 Enrolled Pupils\n• Total Staff & Teachers: 70 Verified Accounts\n• Universal Password for all demo accounts: password123\n• Generated: ${new Date().toLocaleDateString('en-IN', { dateStyle: 'full' })}`, 55, 140);
        ...
        schools.forEach((s, sIdx) => {
            ...
            tY += 16;
            s.teachers.slice(0, 5).forEach((t) => {
