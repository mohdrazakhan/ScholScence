...
let SchoolsService = class SchoolsService {
    ...
    async updateSchoolServices(schoolId, disabledServices) {
        const school = await this.prisma.school.findUnique({
            where: { id: schoolId, deleted_at: null },
        });
        if (!school) {
    ...
    async onboardSchool(dto) {
        ...
        if (existingSchool) {
            throw new common_1.BadRequestException(`A school with code "${normalizedCode}" already exists.`);
        }
        const existingUser = await this.prisma.user.findFirst({
            where: { email: normalizedEmail, deleted_at: null },
        });
        if (existingUser) {
            throw new common_1.BadRequestException(`A user with email "${normalizedEmail}" is already registered.`);
        }
