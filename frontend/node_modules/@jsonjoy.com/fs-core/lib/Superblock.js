...
const endsWithSep = (path, win32) => {
    ...
    if (code === 46)
        code = path.charCodeAt(--i); // "/.", same as a trailing slash
...
class Superblock {
    ...
    constructor(opts = {}) {
        ...
        this.open = (filename, flagsNum, modeNum, resolveSymlinks = true) => {
            ...
            if (!file)
                throw (0, errors_1.createError)("ENOENT" /* ERROR_CODE.ENOENT */, 'open', filename);
            return file.fd;
        };
        ...
        this.read = (fd, buffer, offset, length, position) => {
            if (offset < 0)
                throw new errors.RangeError('ERR_OUT_OF_RANGE', 'offset', '>= 0 && <= 9007199254740991', offset);
            if (length === 0)
                return 0;
            if (length < 0)
                throw new errors.RangeError('ERR_OUT_OF_RANGE', 'length', '>= 0', length);
            ...
            if (file.node.isSymlink()) {
                throw (0, errors_1.createError)("EPERM" /* ERROR_CODE.EPERM */, 'read', file.link.getPath());
            }
        ...
        this.link = (filename1, filename2) => {
            ...
            try {
                ...
                if (err.code)
                    err = (0, errors_1.createError)(err.code, 'link', filename1, filename2);
                throw err;
            }
            ...
            if (dir2.getChild(name))
                throw (0, errors_1.createError)("EEXIST" /* ERROR_CODE.EEXIST */, 'link', filename1, filename2);
        ...
        this.unlink = (filename) => {
            ...
            if (link.getNode().isDirectory())
                throw (0, errors_1.createError)("EPERM" /* ERROR_CODE.EPERM */, 'unlink', filename);
        ...
        this.symlink = (targetFilename, pathFilename) => {
            if (!targetFilename)
                throw (0, errors_1.createError)("ENOENT" /* ERROR_CODE.ENOENT */, 'symlink', targetFilename, pathFilename);
            ...
            try {
                ...
                if (err.code)
                    err = (0, errors_1.createError)(err.code, 'symlink', targetFilename, pathFilename);
                throw err;
            }
            const name = pathSteps[pathSteps.length - 1];
            // Check if new file already exists.
            if (dirLink.getChild(name))
                throw (0, errors_1.createError)("EEXIST" /* ERROR_CODE.EEXIST */, 'symlink', targetFilename, pathFilename);
            ...
            if (!node.canExecute() || !node.canWrite())
                throw (0, errors_1.createError)("EACCES" /* ERROR_CODE.EACCES */, 'symlink', targetFilename, pathFilename);
            // Create symlink.
        ...
        this.rename = (oldPathFilename, newPathFilename) => {
            ...
            try {
                ...
                if (err.code)
                    err = (0, errors_1.createError)(err.code, 'rename', oldPathFilename, newPathFilename);
                throw err;
            }
            ...
            if (!oldLinkParent)
                throw (0, errors_1.createError)("EINVAL" /* ERROR_CODE.EINVAL */, 'rename', oldPathFilename, newPathFilename);
            ...
            if (!oldParentNode.canExecute() ||
                ...
                throw (0, errors_1.createError)("EACCES" /* ERROR_CODE.EACCES */, 'rename', oldPathFilename, newPathFilename);
            }
        ...
        this.mkdir = (filename, modeNum) => {
            ...
            if (!steps.length)
                throw (0, errors_1.createError)("EEXIST" /* ERROR_CODE.EEXIST */, 'mkdir', filename);
            ...
            if (!node.canWrite() || !node.canExecute())
                throw (0, errors_1.createError)("EACCES" /* ERROR_CODE.EACCES */, 'mkdir', filename);
        ...
        this.mkdirp = (filename, modeNum) => {
            ...
            for (i; i < steps.length; i++) {
                ...
                if (node.isDirectory()) {
                    ...
                    throw (0, errors_1.createError)("ENOTDIR" /* ERROR_CODE.ENOTDIR */, 'mkdir', filename);
                }
                created = true;
        ...
        this.rmdir = (filename, recursive = false) => {
            ...
            if (link.length && !recursive)
                throw (0, errors_1.createError)("ENOTEMPTY" /* ERROR_CODE.ENOTEMPTY */, 'rmdir', filename);
            this._emitDeleteRecursive(link);
            this.deleteLink(link);
        };
        ...
        while (i < steps.length) {
            ...
            if (node.isDirectory()) {
                if (checkAccess && !node.canExecute(uid, gid)) {
                    return (0, result_1.Err)((0, errors_1.createStatError)("EACCES" /* ERROR_CODE.EACCES */, funcName, filename));
                }
            }
            else {
                if (i < steps.length - 1) {
                    return (0, result_1.Err)((0, errors_1.createStatError)("ENOTDIR" /* ERROR_CODE.ENOTDIR */, funcName, filename));
                }
            }
            ...
            if (!curr)
                if (checkExistence) {
                    return (0, result_1.Err)((0, errors_1.createStatError)("ENOENT" /* ERROR_CODE.ENOENT */, funcName, filename));
                }
                else {
            ...
            if (node.isSymlink() && (resolveSymlinks || i < steps.length - 1)) {
                if (++hops > MAX_SYMLINK_HOPS)
                    return (0, result_1.Err)((0, errors_1.createStatError)("ELOOP" /* ERROR_CODE.ELOOP */, funcName, filename));
                if (budget)
                    budget.hops = hops;
                const resolvedPath = (0, path_1.isAbsolute)(node.symlink) ? node.symlink : (0, fs_node_utils_2.pathJoin)((0, path_1.dirname)(curr.getPath()), node.symlink); // Relative to symlink's parent
    ...
    getLinkAsDirOrThrow(filename, funcName) {
        ...
        if (!link.getNode().isDirectory())
            throw (0, errors_1.createError)("ENOTDIR" /* ERROR_CODE.ENOTDIR */, funcName, filename);
        return link;
    }
    ...
    getFileByFdOrThrow(fd, funcName) {
        ...
        if (!file)
            throw (0, errors_1.createError)("EBADF" /* ERROR_CODE.EBADF */, funcName);
        return file;
    }
    _toJSON(link = this.root, json = {}, path, asBuffer) {
        ...
        for (const name of children.keys()) {
            ...
            if (node.isFile()) {
                ...
                if (path)
                    filename = (0, fs_node_utils_2.pathRelative)(path, filename);
        ...
        if (path)
            dirPath = (0, fs_node_utils_2.pathRelative)(path, dirPath);
        if (dirPath && isEmpty) {
            json[dirPath] = null;
        }
        return json;
    ...
    openLink(link, flagsNum, resolveSymlinks = true, filename = link.getPath()) {
        if (this.openFiles >= this.maxFiles) {
            // Too many open files.
            throw (0, errors_1.createError)("EMFILE" /* ERROR_CODE.EMFILE */, 'open', filename);
        }
        // Resolve symlinks.
        //
        ...
        if (node.isDirectory()) {
            if ((flagsNum & (O_RDONLY | O_RDWR | O_WRONLY)) !== O_RDONLY)
                throw (0, errors_1.createError)("EISDIR" /* ERROR_CODE.EISDIR */, 'open', filename);
        }
        else {
            if (flagsNum & O_DIRECTORY)
                throw (0, errors_1.createError)("ENOTDIR" /* ERROR_CODE.ENOTDIR */, 'open', filename);
        }
        if (node.isSymlink() && flagsNum & O_NOFOLLOW)
            throw (0, errors_1.createError)("ELOOP" /* ERROR_CODE.ELOOP */, 'open', filename);
        ...
        if ((flagsNum & (O_RDONLY | O_RDWR | O_WRONLY)) !== O_WRONLY) {
            if (!node.canRead()) {
                throw (0, errors_1.createError)("EACCES" /* ERROR_CODE.EACCES */, 'open', filename);
            }
        }
    ...
    throwCreatDir(path, filename, budget) {
        ...
        if (!dir.ok)
            throw (0, errors_1.createError)(dir.err.code, 'open', filename);
    ...
    openFile(filename, flagsNum, modeNum, resolveSymlinks = true) {
        ...
        if ((flagsNum & (O_CREAT | O_DIRECTORY)) === (O_CREAT | O_DIRECTORY))
            throw (0, errors_1.createError)("EINVAL" /* ERROR_CODE.EINVAL */, 'open', filename);
        const win32 = this.process.platform === 'win32';
        const budget = { hops: 0 };
        let trailingSlash = endsWithSep(filename, win32);
        if (trailingSlash && flagsNum & O_CREAT)
            this.throwCreatDir(filename, filename, budget);
        ...
        for (;;) {
            ...
            if (!result.ok) {
                ...
                if (code !== "ENOENT" /* ERROR_CODE.ENOENT */ || !(flagsNum & O_CREAT))
                    throw (0, errors_1.createError)(code, 'open', filename);
                budget.hops = hops;
                link = null;
                break;
            }
            link = result.value;
            const node = link.getNode();
            if (!follow || !node.isSymlink())
                break;
            if (++budget.hops > MAX_SYMLINK_HOPS)
                throw (0, errors_1.createError)("ELOOP" /* ERROR_CODE.ELOOP */, 'open', filename);
            ...
            if (endsWithSep(path, win32)) {
                if (flagsNum & O_CREAT)
                    this.throwCreatDir(path, filename, budget);
                trailingSlash = true;
            }
        }
        if (link) {
            const node = link.getNode();
            if (excl)
                throw (0, errors_1.createError)("EEXIST" /* ERROR_CODE.EEXIST */, 'open', filename);
            if (trailingSlash && !node.isDirectory())
                throw (0, errors_1.createError)("ENOTDIR" /* ERROR_CODE.ENOTDIR */, 'open', filename);
            if (flagsNum & O_CREAT && node.isDirectory())
                throw (0, errors_1.createError)("EISDIR" /* ERROR_CODE.EISDIR */, 'open', filename);
    ...
    write(fd, buf, offset, length, position) {
        ...
        if (file.node.isSymlink()) {
            throw (0, errors_1.createError)("EBADF" /* ERROR_CODE.EBADF */, 'write', file.link.getPath());
        }
